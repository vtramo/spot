...
int main(int argc, char** argv)
{
  ...
  for (auto model : models)
  {
    ...
    // XXX: we allow for only 80% memory based on what we should use to explore the full graph
    std::size_t allowed_mem = model.nb_states * 0.8f;
    std::cout << model.path << " " << model.nb_states << " " << allowed_mem << "\n";
    run_one_reachability_bench(sys, allowed_mem, 0);
    for (int i = 10; i < 100; i += 5)
    {
      std::size_t hash_set_size = allowed_mem * (double) i / 100.;
      std::size_t bloom_filter_size = allowed_mem - hash_set_size;

      run_one_reachability_bench(sys, hash_set_size, bloom_filter_size);
    }
  }
