...
The key =strongX= refers to the =X[!]= operator of LTLf.  It is
disabled by default, but you may use =--ltl-priorities 'strongX=1'= to
enable it.
...
priorities used to generate PSL formulas requires three different
options:
