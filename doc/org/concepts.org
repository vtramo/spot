...
| ~attr-strategy~     | ~std::vector<int>~             | vector representing the attractorlvl for each state. A strategy can be deduced from this by either decreasing the lvl (current lvl is strictly positive) or going to any winning state if the current lvl is 0.     |
...
| ~outcond-edge~      | ~std::vector<bdd>~             | bdd minterm over synthesis-outputs defining the outs of each edge (used by ~print_aiger()~ for instance)                                                                                                            |
| ~transposed  ~      | ~spot::transposed_graph_t~     | Transposed arena for solving games efficiently                                                                                                                                                                      |
