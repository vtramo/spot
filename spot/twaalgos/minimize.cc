...
namespace spot
{
  ...
  void minimize_mealy_fast_here(twa_graph_ptr&, bool)
  {
    return;
  }
}
...
namespace // Anonymous for mealy_min
{
...
//  }
  ...
  trans_comp_classes(const square_matrix<bool, true>& incompmat)
  {
    ...
    std::vector<bool> treated(n_env, false);
    ...
    for (unsigned s = 0; s < n_env; ++s)
      {
        ...
        ret.push_back(std::vector<bool>(n_env, false));
        auto& this_group = ret.back();
        this_group[s] = true;
        ...
        while (!stack_.empty())
          {
            ...
            for (unsigned scp = s + 1; scp < n_env; ++scp)
              {
                if (treated[scp] || incompmat.get(sc, scp))
                  continue;
                treated[scp] = true;
                this_group[scp] = true;
                           ...
                           const unsigned n_env)
  {
    ...
    const unsigned n_groups = red.groups.size();
    for (unsigned groupidx = 0; groupidx < n_groups; ++groupidx)
      {
        auto& group = red.groups[groupidx];
        all_bdd.clear();
...

  {
    ...
    const auto& group = red.groups.at(i);
    const auto& mlv = red.minimal_letters_vec.at(i);
    const unsigned n_ml = mlv.size();
    const unsigned nsg = std::count(group.begin(), group.end(), true);
                                 ...
                                 const unsigned n_env)
  {
    ...
    print_hoa(std::cerr, split_mmw) << '\n';
    {
      ...
      for (unsigned group = 0; group < red.groups.size(); ++group)
        {
                            ...
                            [&, cc = e.cond](const auto& bs_idx_vec)
                              {return cc
                                  == red.minimal_letters_vec[group]
                                       [bs_idx_vec.front()]; }));
//                        find_first_index_of(red.minimal_letters_vec[group],
//                                            [cc=e.cond](const auto&c)
//                                            {return cc == c; }));
            }
        }
                   ...
                   stopwatch& sw)
  {
    ...
    red.groups = trans_comp_classes(incompmat);
    dotimeprint << "Done trans comp " << red.groups.size()
                << " - " << sw.stop() << '\n';
...
//    std::vector<int8_t>
    ...
    get_sol()
    {
      ...
      // The vector is empty iff the the prob is unsat
      ...
      switch (picosat_sat(lm.psat_, -1))
      {
        ...
        case(PICOSAT_SATISFIABLE):
          {
            std::vector<int> res(1 + picosat_variables(lm.psat_), -1);
            res[0] = 0; // Convention
            for (int lit : lm.all_lits)
              res.at(lit) = picosat_deref(lm.psat_, lit);
#ifdef TRACE
            trace << "Sol is \n";
...
//          mm_pb.add_static(-lm.sxi2lit({x, iaj.i}));
...
//          {
//            mm_pb.add_static(lm.sxi2lit({xprime, iaj.j}));
//            trace << lm.sxi2lit({xprime, iaj.j}) << ' ';
//          }
        *(++clause_it) = 0;
...
//        const unsigned n_letters_g = red.minimal_letters_vec[group].size();
        ...
        for (unsigned src_class = 0; src_class < n_classes; ++src_class)
          if (red.groups[group][psolv[src_class]])
            {
              unsigned idx =
        ...
        for (unsigned abddidu = 0; abddidu < n_letters_g; ++abddidu)
          {
//            const bdd& abdd = red.minimal_letters_vec[group][abddidu];
                ...
                for (unsigned src_class = 0; src_class <= new_class;
                     ++src_class)
                  {
                    ...
                    if (src_class < n_psol && psolv[src_class] == x)
                      ...
                      // c) Full condition
                      {
                        int sxi = lm.get_sxi({x, src_class});
                        assert(sxi);
//                        inc_clause[2] = -lm.sxi2lit({x, src_class});
                ...
                for (unsigned dst_class = 0; dst_class < new_class;
                     ++dst_class)
                  {
                    ...
                    if (dst_class < n_psol
                      ...
                      {
  //                      inc_clause[2] = lm.sxi2lit({xprime, dst_class});
                           ...
                           decltype(iaj_eq)>& used_ziaj_map)
  {
    ...
    auto get_player = [&](unsigned dst_class, const bdd& outcond)
      {
//        static unsigned next_state = minmach->num_states();
        auto [it, inserted] =
                        ...
                        stopwatch& sw)
  {
    ...
    for (unsigned i = 0; i < n_classes; ++i)
      {
        ...
        x_in_class[i].first =
            find_first_index_of(red.groups,
                                [first_x](const auto& v)
                                    {return v[first_x]; });
      }
#ifdef TRACE
    for (unsigned i = 0; i < n_classes; ++i)
    ...
    assert(std::all_of(x_in_class.begin(), x_in_class.end(),
                       ...
                       {
                         const auto& g = red.groups[p.first];
                         for (unsigned i = 0; i < n_env; ++i)
                           if (p.second[i] && !g.at(i))
                             {
                               trace << "state " << i << " is in a class "
                                                         "associated to group "
                                     << p.first << std::endl;
                               return false;
                             }
              ...
              if (sol.at(lm.get_iaj({src_class, amlbidu, dst_class})) == 1)
                {
                  ...
                  for (unsigned abddidu : red.bisim_letters[group][amlbidu])
                    {
                      used_ziaj_map[{src_class, abddidu, dst_class}] = bddtrue;
                             ...
                             ++src_class)
                          {
                            ...
                            for (unsigned dst_class = 0;
                                 ...
                                 ++dst_class)
                              {
                                ...
                                // but we still loop over all if changed later
                                ...
                                if (it != used_ziaj_map.end())
                                  {
                                    ...
                                    it->second &= outcond;
                                    assert(it->second != bddfalse
                                           && "Probably incompatible states "
                                              "in the same class");
                                  }
                              } // dst_class
                          } // src_class
...
namespace spot
{
  twa_graph_ptr minimize_mealy(const const_twa_graph_ptr& mm,
                               int preminfast, bool keep_split)
    {
      stopwatch sw;
      sw.start();
