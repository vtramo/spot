...
namespace
{
                            ...
                            bool sorted, bool split)
  {
    ...
    for (const auto& ap : bdd_part.orig_ap())
      aut.unregister_ap(aut.register_ap(ap));
    // Register new ones
    for (const auto& ap : bdd_part.new_ap())
      aut.register_ap(ap);
...
namespace spot
{
  ...
  bdd_partition::tidy_up_()
  {
    ...
    for (unsigned lidx = 0; lidx < nleaves; ++lidx)
      {
        ...
        auto [it, ins] = leaves_classes.emplace(all_parents[lidx],
                                              leaves_classes.size());
        (void) ins;
    ...
    for (const auto& elem : leaves_classes)
      {
        const auto& lcidx = elem.second;
